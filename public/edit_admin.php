...
<?php 
//find_all_admins();
//$admin=find_admin_with_id($admin["id"]);
...
$output1 = "Sorry, we don't recognize the value that you entered , XSS attack";
	 $output2 = "Sorry, we don't recognize the value that you entered , SQL attack";
	  $output5 = "Sorry, we don't recognize the value that you entered , LFI attack";
...
<?php
   ...
   ?>
<h1>Edit admin
<?php echo htmlentities($admin["username"]); ?></h1>
<form action="http://localhost/p/public/master.php?page=edit_admin.php&admin=<?php echo urlencode($admin["id"]) ;?>" method="post">
   ...
   <a href="http://localhost/p/public/master.php?page=manage_admins.php">Cancel </a>
  </div>
</div>
...
if(isset($_POST['submit'])){
	...
	$field=array("username" => 30);
	validate_max_length($field);
	if(empty($errors)){
...
mysqli_stmt_bind_param($stmt, "ssi", $username,$pass,$id);
